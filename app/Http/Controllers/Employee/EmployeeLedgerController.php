...
class EmployeeLedgerController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            $voucherNo = DB::table('invoiceno')->first('voucher_no');
            $getVoucherNo = $voucherNo->voucher_no;
            $purchaseNumber = 'VOU' . str_pad($getVoucherNo, 6, '0', STR_PAD_LEFT);
            
            DB::table('invoiceno')->update([
                'voucher_no' => $getVoucherNo + 1,
            ]);
            
            $input['voucher_no'] = $purchaseNumber;
            // dd($input);
            ...
            DB::rollback();
            return back()->withErrors(['error' => 'Ops ! Could Not add ledger history. DB transaction lost']);
        }
