...
class EmployeeLeaveEntryController extends Controller
{
    ...
    // public function employeeLeaveEntryListTable()
    // {
    //     $employeeDetails = EmployeeLeaveEntry::select('employee_leave_entries.*')
    //                     ->with(['employeeId:id,employee_name', 'alternativeEmployeeId:id,employee_name'])
    //                     ->orderBy('id', 'desc')
    ...
    public function leaveEntryListSearch($branchID, $employeeID, $startDate, $endDate, $pdf)
    {
        $query = "
        SELECT
            le.id AS leaveID,
            (SELECT branch_name FROM emp_branches WHERE id = le.branch_id) AS BranchName,
            ...
            IFNULL(le.reason_for_leave, '') AS Reason,
            IF(le.department_status = 1, 'Approved', 'Pending') AS DepartmentStatus,
            IF(le.hr_status = 1, 'Approved', 'Pending') AS HRStatus,
            IF(le.management_status = 1, 'Approved', 'Pending') AS ManagementStatus,
        ...
        WHERE
            ...
            AND (le.branch_id = $branchID OR $branchID = 0)
            AND (le.employee_id = $employeeID OR $employeeID = 0)
        ";
        $employeeDetails = DB::table(DB::raw("($query) AS subquery"))
                ->select('leaveID', 'BranchName', 'EmployeeName', 'AlternativeEmployeeName','LeaveType','LeaveStartDate','LeaveEndDate','TotalDays','Reason','DepartmentStatus', 'HRStatus', 'ManagementStatus', 'FinalStatus')
                ->orderBy('leaveID')
                ->get();
    ...
    public function employeeLeaveEntryList()
    {
        ...
        return view('pages.employee.employee_leave_entry.leave_entry_list', compact( 'startDate', 'endDate', 'employees', 'allEmpBranch'));
    }
    ...
    public function branchWaisEmployeeID($branchID)
    {
