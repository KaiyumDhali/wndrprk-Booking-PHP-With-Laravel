...
class ProductServiceController extends Controller
{
    ...
    public function __productServicesEntry($id)
    {
        $warehouses = Warehouse::where('status', 1)->get();
        $products = Product::where('is_saleable', 1)->where('status', 1)->get();
        ...
        return view('pages.product.product_services.service_entry', compact('getProductServicesEntry', 'warehouses', 'products', 'customerAccounts', 'toAccounts'));
    }
    public function __productServicesEntryStore(Request $request, $id)
    {
        ...
        // Find the product service detail record by ID
        $productServiceDetail = ProductServiceDetail::find($id);

        // Check if the record exists
        if (!$productServiceDetail) {
            return redirect()->route('product_services')->with('error', 'Service detail not found.');
        }

        // Update the product service detail fields with validated data
        $productServiceDetail->actual_service_date = $request->actual_service_date;
        $productServiceDetail->service_man_name = $request->service_man_name;
        $productServiceDetail->service_man_mobile = $request->service_man_mobile;
        $productServiceDetail->remarks = $request->remarks;
        ...
        return redirect()->route('pending_product_services')->with('success', 'Services assigned successfully.');
    }
