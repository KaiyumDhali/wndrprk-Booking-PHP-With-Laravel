...
class SalesController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            foreach ($request->get('table_product_id') as $key => $productId) {
                ...
                $salesProductData[] = "Name:$productName Qty:$stockQuantity X $stockPrice\n";
            }
            // $salesProductData[] = "Invoice No: $salesNumber, $remarks\n";
            ...
            if ($givenAmount > 0) {
                ...
                if ($bank_name) {
                    $narration[] = "Invoice:$salesNumber, $payAccName Received From: $customerName, Through Bank:$bank_name, Branch:$branch_name, A/C:$ac_no, Cheque No:$cheque_no, Cheque Date:$cheque_date, Paid Amount:$formatAmount TK\n";
                    $payment_type = "Bank";
                } else if ($mobile_bank_name) {
                    $narration[] = "Invoice:$salesNumber, $payAccName Received From: $customerName, Through Mobile Bank:$mobile_bank_name, Transaction ID:$transaction_id, Paid Amount:$formatAmount TK\n";
                    $payment_type = "mBank";
                } else {
                    $narration[] = "Invoice:$salesNumber, $payAccName Received From: $customerName, Through Cash Paid Amount:$formatAmount TK\n";
                    $payment_type = "Cash";
                }
                $narration = implode("\n", $narration);
                echo $narration;
        ...
        //     'message' => 'Sales History Added Successfully!',
        //     'alert-type' => 'success'
        // ])->with('pdf_url', route('sales_invoice_details_pdf', $salesNumber));
    }
    ...
    // {
    //     $inv = 'INV';
    //     $query = Stock::selectRaw('invoice_no, customer_id, MIN(stock_date) as stock_date, SUM(stock_out_discount) as total_discount, SUM(stock_out_total_amount) as total_amount')
    //         ->where('invoice_no', 'LIKE', '%' . $inv . '%');

    //     if ($request->startDate != 0 && $request->endDate != 0) {
    //         $query->whereBetween('stock_date', [$request->startDate, $request->endDate]);
    //     }
    //     $salesInvoices = $query->groupBy('invoice_no', 'customer_id')
    //         ->orderBy('id', 'DESC')
    //         ->get();
    //     return response()->json($salesInvoices);
    // }
    ...
    public function salesInvoiceDetails($invoiceNo)
    {
        ...
        return view('pages.product.stock.sales_report_invoice_details', compact('stocks', 'customerPayment', 'paymentNarration'));
    }
    ...
    public function salesInvoiceDetailsPdf($invoices)
    {
        ...
        $pdf = PDF::loadView('pages.pdf.sales_report_invoice_wise_pdf', array('stocks' => $stocks, 'invoices' => $invoices, 'customerPayment' => $customerPayment, 'paymentNarration' => $paymentNarration, 'data' => $data));
        $pdf->setPaper('A4', 'portrait');
        return $pdf->stream(Carbon::now() . '-sales_report.pdf');
    }
