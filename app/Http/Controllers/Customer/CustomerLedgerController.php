...
class CustomerLedgerController extends Controller {
    ...
    public function customerDetails($id)
    {
        ...
        $customerDetails = CustomerLedger::where('customer_id', $id)->get();
    ...
    {
        $query = "
            SELECT -1 as id,-1 as customer_id, '$startDate' as ledger_date,'Opening Balance on : $startDate' AS invoice_no,IFNULL( SUM(credit),0) as credit ,IFNULL(SUM(debit),0) as debit,'Opening Balance' as remarks
            FROM `customer_ledgers` 
            WHERE customer_id = '$customerID' AND ledger_date <'$startDate'
            UNION ALL 
            SELECT id,customer_id,ledger_date,invoice_no,credit,debit,remarks 
            FROM `customer_ledgers` 
            WHERE customer_id = '$customerID' AND ledger_date BETWEEN '$startDate' AND '$endDate'
            ";
        ...
        if ($pdf == "pdfurl") {
            ...
            $data['customerInfo'] = Customer::where('id',$customerID)->select('id','customer_code', 'customer_name', 'customer_mobile', 'customer_email', 'customer_address')->first();

            $pdf = PDF::loadView('pages.pdf.customer_ledger_report_pdf', array('customerLedgers' => $result, 'data' => $data));
