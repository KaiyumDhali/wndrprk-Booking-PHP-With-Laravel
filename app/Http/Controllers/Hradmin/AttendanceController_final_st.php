...
class AttendanceController extends Controller {
    ...
    public function dailyAllAttendance()
    {
        ...
        $lateTime = '09:37:00';
        $query = "
            select *, DATE_FORMAT(TIMEDIFF('$lateTime', a.InTime),'%H:%i:%s') AS LateTime, DATE_FORMAT(TIMEDIFF(a.OutTime, a.InTime),'%H:%i:%s') AS WorkTime from(
                SELECT
                    ...
                    IF( (SELECT COUNT(ed.id) FROM employee_delayin_earlyouts ed WHERE ed.employee_id = employees.id
                        AND ed.date = t._date
                        AND ed.status = 'out_of_office') > 0, 'out_of_office',
                        IF( (SELECT COUNT(ed.id) FROM employee_delayin_earlyouts ed WHERE ed.employee_id = employees.id
                            AND ed.date = t._date
                            AND ed.status = 'absences') > 0, 'absences',
        ...
        select COUNT(ID) AS TotalEmployees,(SUM(t.present)+SUM(t.out_of_office))AS Present, SUM(t.out_of_office) AS out_of_office, SUM(t.late_in) AS late_in,SUM(t.monthly_leave) AS monthly_leave, t.holiday AS holiday, t.weekend AS weekend  from(
            SELECT
                ...
                DAYNAME(v._date) AS Day,
                    ...
                    IFNULL( IF(TIME((SELECT DATE_FORMAT(date_time_record, '%H:%i:%s') FROM attendances WHERE employee_id = employees.employee_code AND date_only_record = v._date ORDER BY id ASC LIMIT 1)) > '$lateTime',1,0), 0) AS late_in,
                    ...
                    if(DAYNAME(v._date) IN('Friday', 'Saturday'),1,0) AS weekend
            FROM
                employees
                LEFT JOIN emp_departments ON employees.department_id = emp_departments.id
                LEFT JOIN emp_designations ON employees.designation_id = emp_designations.id
