...
class FinanceTransactionController extends Controller
{
    ...
    // {
    //     $accountNameList = FinanceAccount::pluck('account_name', 'id')->all();
    //     // $accountNameList = FinanceAccount::where('account_status', 1)->where('account_group_code', 'like', '10002%')->whereNotIn('account_group_code', ['100020001'])->pluck('account_name', 'id');
    //     // return response()->json($accountNameList);
    //     return response()->json([
    //         'accountNameList' => $accountNameList,
    //     ]);
    // }
    // Date Between Search receivedVoucher list
    // public function __receivedVoucherDateSearch(Request $request)
    // {
    ...
    //     $inv = '01CR';
    //     $SVInv = '01SV';
    //     $vouchers = FinanceTransaction::where(function ($query) use ($startDate, $endDate) {
    //         if ($startDate && $endDate && $startDate !== '0' && $endDate !== '0') {
    //             $query->whereRaw("DATE(voucher_date) BETWEEN '$startDate' AND '$endDate'");
    //         }
    //     })
    //         ->where('voucher_no', 'LIKE', '%' . $inv . '%')
    //         ->where('voucher_no', 'LIKE', '%' . $SVInv . '%')
    //         ->orderBy('id', 'DESC')
    //         ->get()
    //         ->groupBy('voucher_no');
    //     return response()->json($vouchers);
    // }
    // receivedVoucherDateSearch



    public function receivedVoucherDateSearch(Request $request)
    {
        $startDate = $request->startDate;
        $endDate = $request->endDate;
        // Invoice Type
    ...
    // Date Between Search receivedVoucher list
    public function paymentVoucherDateSearch(Request $request)
{
    $startDate = $request->startDate;
    $endDate = $request->endDate;
    // Invoice Type
