...
class FinanceTransactionController extends Controller
{
    ...
    {
        return view('pages.finance.report.summery_report');
    }

    public function summaryReportSearch($startDate, $endDate, $pdf)
    {

       $getTotalSummeryReport = DB::connection()->select("CALL sp_GetTotalSummeryReport(?, ?)", array($startDate, $endDate));
       $getTotalProfit = DB::connection()->select("CALL sp_TotalProfit(?, ?)", array($startDate, $endDate));


        if ($pdf == "list") {
            // return response()->json($getTotalSummeryReport);
            return response()->json([
                'totalSummeryReport' => $getTotalSummeryReport,
        ...
        if ($pdf == "pdfurl") {
            ...
            $pdf = PDF::loadView('pages.pdf.summeryReport_pdf', array('getTotalSummeryReport' => $getTotalSummeryReport, 'data' => $data));
            // $pdf->setPaper('A4', 'landscape');
            return $pdf->stream(Carbon::now() . '-summeryReport.pdf');
        }
    }
    
    // accountNameList
    public function accountList($code)
    {
        $acList = FinanceAccount::where('account_status', 1)->where('account_group_code', $code)->pluck('account_name', 'id');
        ...
        return response()->json(['acList' => $acList,]);
    }
    // accountNameList
    ...
    public function journalVoucher()
    {
        ...
        return view('pages.finance.voucher.journal_voucher', compact('accountList', 'startDate', 'endDate'));
    }
    // journal Stor 
    public function journalVoucherStore(Request $request)
    {
        ...
        $this->validate($request, [
            ...
            'account_names'  => 'required|array|min:2',
            'balance_type'   => 'required|array|min:2',
            'amount'         => 'required|array|min:2',
        ...
        $balanceTypes = $request->input('balance_type');
        $amounts = $request->input('amount');
        $narration = $request->input('narration');
        $doneBy = Auth::user()->name;

        // Ensure the sum of credit and debit amounts are equal
        $totalCredit = 0;
        $totalDebit = 0;
        foreach ($balanceTypes as $index => $type) {
            if ($type === 'Cr') {
                $totalCredit += $amounts[$index];
            } elseif ($type === 'Dr') {
                $totalDebit += $amounts[$index];
            }
        }

        if ($totalCredit !== $totalDebit) {
            return back()->withErrors(['error' => 'Total Credit and Debit amounts must be equal.'])->withInput();
        }
        ...
        try {
            ...
            // Create Finance Transactions with matched Cr and Dr accounts
            foreach ($balanceTypes as $index => $type) {
                if ($type === 'Cr') {
                    // Find corresponding Dr entry
                    $drIndex = array_search('Dr', $balanceTypes);
                    if ($drIndex === false) {
                        throw new \Exception('No matching Dr entry found for Cr account.');
                    }
                    ...
                    FinanceTransaction::create([
                        ...
                        'acid'              => $accountIds[$index],         // Cr account ID
                        'to_acc_name'       => $accountNames[$drIndex],     // Corresponding Dr account name
                        'type'              => 'JV',
                        'amount'            => $amounts[$index],
                        'balance_type'      => $balanceTypes[$index],
                        'payment_type'      => 'Journal',
                        'narration'         => $narration,
                        'transaction_date'  => $voucherDate,
                        'transaction_by'    => $doneBy,
                        'done_by'           => $doneBy,
                        'updated_by'        => $doneBy,
                    ]);

                    // Now create the corresponding Dr transaction
                    FinanceTransaction::create([
                        'company_code'      => '01',
                        'voucher_no'        => $voucherNo,
                        'voucher_date'      => $voucherDate,
                        'acid'              => $accountIds[$drIndex],       // Dr account ID
                        'to_acc_name'       => $accountNames[$index],       // Corresponding Cr account name
                        'type'              => 'JV',
                        'amount'            => $amounts[$drIndex],
                        'balance_type'      => $balanceTypes[$drIndex],
                        'payment_type'      => 'Journal',
                        'narration'         => $narration,
                        'transaction_date'  => $voucherDate,
                        'transaction_by'    => $doneBy,
            ...
            DB::rollback();
            \Log::error('Journal Voucher Error: ' . $e->getMessage());
            return back()->withErrors(['error' => 'An error occurred. Please try again.'])->withInput();
        }

        return back()->with([
            'message' => 'Journal Voucher Entry Added Successfully!',
            'alert-type' => 'success'
        ]);
    }

    // received voucher search list and PDF
    public function journalVoucherDateSearch($startDate, $endDate, $pdf)
    {
        // Invoice Type
        $inv = '01JV';
        // $SVInv = '01SV';
        ...
        $receivedVouchersQuery = FinanceTransaction::join('finance_accounts', 'finance_transactions.acid', '=', 'finance_accounts.id')
            ...
            ->where('voucher_no', 'LIKE', '%' . $inv . '%');  // Match only $inv

        // Handle JSON response
        if ($pdf == "list") {
            $receivedVouchers = $receivedVouchersQuery->orderBy('finance_transactions.voucher_date', 'DESC')->orderBy('finance_transactions.id', 'DESC')->get();
        ...
        if ($pdf == "pdfurl") {
            ...
            $pdf = PDF::loadView('pages.pdf.journal_voucher_date_pdf', ['receivedVouchers' => $receivedVouchers, 'data' => $data]);
            $pdf->setPaper('A4', 'portrait');
            return $pdf->stream(Carbon::now() . '-journal_voucher_date_pdf.pdf');
        }
    ...
    {
        $voucherData = DB::table('invoiceno')->first(['voucher_no']);
        $currentVoucherNo = $voucherData->voucher_no;
        $newVoucherNo = '01JV' . str_pad($currentVoucherNo, 6, '0', STR_PAD_LEFT);

        DB::table('invoiceno')->update(['voucher_no' => $currentVoucherNo + 1]);
    ...
    public function receivedVoucherStore(Request $request)
    {
        ...
        $payAccGroupName = FinanceAccount::leftJoin('finance_groups', 'finance_accounts.account_group_code', '=', 'finance_groups.group_code')
            ...
            ->where('finance_accounts.id', $acid)
            ->value('finance_groups.group_name');
        // dd($payAccGroupName);
        //$payment_type = $request->input('payment_type');
        $payment_type = $payAccGroupName;
        // Bank Account   Mobile Bank   Cash in hand
        $formatAmount = formatCurrency($amount);
        // $narration[] = "$acid_name Received From: $to_acc_name, Through $payAccGroupName Received Amount:$formatAmount TK\n";
        if ($payAccGroupName == 'Bank Account') {
            if ($cheque_type == 'Cheque') {
                $narration[] = "$acid_name Received From: $to_acc_name, Through Bank $cheque_type, Cheque No:$cheque_no, Cheque Date:$cheque_date, Received Amount:$formatAmount TK $remarks\n";
            } else {
                $narration[] = "$acid_name Received From: $to_acc_name, Through Bank $cheque_type, Received Amount:$formatAmount TK $remarks\n";
            }
        } else if ($payAccGroupName == 'Mobile Bank') {
            $narration[] = "$acid_name Received From: $to_acc_name, Through Mobile Bank:$mobile_bank_name, Mobile Number: $mobile_number, Transaction ID:$transaction_id, Received Amount:$formatAmount TK $remarks\n";
        } else {
            $narration[] = "$acid_name Received From: $to_acc_name, Through Cash Received Amount:$formatAmount TK $remarks\n";
        }
        $narration = implode("\n", $narration);
        echo $narration;
    ...
    public function paymentVoucherDateSearch($startDate, $endDate, $pdf)
    {
        ...
        // Handle JSON response
        if ($pdf == "list") {
            $paymentVouchers = $paymentVouchersQuery->orderBy('finance_transactions.voucher_date', 'DESC')->orderBy('finance_transactions.id', 'DESC')->get();
    ...
    public function paymentVoucherStore(Request $request)
    {
        ...
        if ($payAccGroupName == 'Bank Account') {
            if ($cheque_type == 'Cheque') {
                $narration[] = "$acid_name Payment To: $to_acc_name, Through Bank $cheque_type, Cheque No:$cheque_no, Cheque Date:$cheque_date, Payment Amount:$formatAmount TK $remarks\n";
            } else {
                $narration[] = "$acid_name Payment To: $to_acc_name, Through Bank $cheque_type, Payment Amount:$formatAmount TK $remarks\n";
            }
        } else if ($payAccGroupName == 'Mobile Bank') {
            $narration[] = "$acid_name Payment To: $to_acc_name, Through Mobile Bank:$mobile_bank_name, Mobile Number: $mobile_number, Transaction ID:$transaction_id, Payment Amount:$formatAmount TK $remarks\n";
        } else {
            $narration[] = "$acid_name Payment To: $to_acc_name, Through Cash, Payment Amount:$formatAmount TK $remarks\n";
        }
        $narration = implode("\n", $narration);
        echo $narration;
    ...
    // Total Customer Receivable List
    public function totalCustomerReceivableList()
    {
        return view('pages.finance.ledger.total_customer_receivable_list');
    }
    // Total Customer Receivable List Search
    public function totalCustomerReceivableListSearch($pdf)
    {
        $query = "
            WITH cte AS (
                SELECT fa.account_name, fa.account_mobile,
                    SUM(CASE WHEN ft.balance_type = 'Dr' THEN ft.amount ELSE 0 END) AS total_dr,
                    SUM(CASE WHEN ft.balance_type = 'Cr' THEN ft.amount ELSE 0 END) AS total_cr
                FROM finance_transactions AS ft
                INNER JOIN finance_accounts AS fa ON ft.acid = fa.id
                WHERE fa.account_group_code = '100020001'
                GROUP BY fa.account_name, fa.account_mobile
            )

            SELECT account_name, account_mobile, total_dr, total_cr, (total_dr - total_cr) AS due
            FROM cte
            WHERE (total_dr - total_cr) > 0
        ";
        $customerReceivableListSearch = DB::table(DB::raw("($query) AS subquery"))
            ->select('account_name', 'account_mobile', 'due')
            ->get();

        if ($pdf == "list") {
            return response()->json($customerReceivableListSearch);
        }
        if ($pdf == "pdfurl") {
            $companySetting = CompanySetting::where('status', 1)->orderBy('id', 'desc')->first();
            $data['company_name'] = $companySetting->company_name;
            ...
            // pdf
            $pdf = PDF::loadView('pages.pdf.total_customer_receivable_list_pdf', array('customerReceivableList' => $customerReceivableListSearch, 'data' => $data));
            // $pdf->setPaper('A4', 'landscape');
            return $pdf->stream(Carbon::now() . '-total_customer_receivable_list_pdf.pdf');
        }
    }

    // Total Supplier Payable List
    public function totalSupplierPayableList()
    {
        return view('pages.finance.ledger.total_supplier_payable_list');
    }
    // Total Supplier Payable List Search
    public function totalSupplierPayableListSearch($pdf)
    {
        $query = "
            WITH cte AS (
                SELECT fa.account_name, fa.account_mobile,
                ...
                FROM finance_transactions AS ft
                INNER JOIN finance_accounts AS fa ON ft.acid = fa.id
                WHERE fa.account_group_code = '400010001'
                GROUP BY fa.account_name, fa.account_mobile
            )

            SELECT account_name, account_mobile, total_dr, total_cr, (total_cr-total_dr) AS due
            FROM cte
            WHERE ( total_cr-total_dr) > 0
        ";
        $supplierPayableListSearch = DB::table(DB::raw("($query) AS subquery"))
            ->select('account_name', 'account_mobile', 'due')
            ->get();

        if ($pdf == "list") {
            return response()->json($supplierPayableListSearch);
        }
        if ($pdf == "pdfurl") {
            $companySetting = CompanySetting::where('status', 1)->orderBy('id', 'desc')->first();
            $data['company_name'] = $companySetting->company_name;
            $data['report_date'] = now()->format('d F Y');
            // pdf
            $pdf = PDF::loadView('pages.pdf.total_supplier_payable_list_pdf', array('supplierPayableList' => $supplierPayableListSearch, 'data' => $data));
            // $pdf->setPaper('A4', 'landscape');
            return $pdf->stream(Carbon::now() . '-total_supplier_payable_list_pdf.pdf');
        }
    }
    ...
    {
        $accountList = FinanceAccount::where('account_status', 1)->where('account_group_code', '100020002')->pluck('account_name', 'id')->all();
        return view('pages.finance.ledger.cash_book_ledger_list', compact('accountList'));
    }
    // cash book ledger Search
    public function cashBookLedgerSearch($accountHead, $startDate, $endDate, $pdf)
    {
        $ac_id = $accountHead;
        $start_date = $startDate;
        $end_date = $endDate;
        $start_date2 = '0001-01-01';
        $end_date2 = Carbon::parse($start_date)->subDay();
        $bank_book = DB::connection()->select("CALL sp_GeneralLedgerByAccount(?, ?, ?, ?, ?)", array($ac_id, $start_date, $end_date, $start_date2, $end_date2));
        //        dd($general_ledger);
        if ($pdf == "list") {
            return response()->json($bank_book);
        }
        if ($pdf == "pdfurl") {
            $accountHead = FinanceAccount::where('id', $ac_id)->first('account_name');
            $companySetting = CompanySetting::where('status', 1)->orderBy('id', 'desc')->first();
            $data['company_name'] = $companySetting->company_name;
            $data['company_address'] = $companySetting->company_address;
            $data['company_logo_one'] = $companySetting->company_logo_one;
            ...
            $pdf = PDF::loadView('pages.pdf.cash_book_pdf', array('bank_book' => $bank_book, 'accountHead' => $accountHead, 'data' => $data));
            // $pdf->setPaper('A4', 'landscape');
            return $pdf->stream(Carbon::now() . '-cash_book_pdf.pdf');
        }
    }

    // bank book ledger
    public function bankBookLedger()
    {
        $accountList = FinanceAccount::where('account_status', 1)->where('account_group_code', '100020004')->pluck('account_name', 'id')->all();
        return view('pages.finance.ledger.bank_book_ledger_list', compact('accountList'));
    }
    // bank book ledger Search
    public function bankBookLedgerSearch($accountHead, $startDate, $endDate, $pdf)
    {
        $ac_id = $accountHead;
        $start_date = $startDate;
        $end_date = $endDate;
        ...
        if ($pdf == "pdfurl") {
            ...
            $pdf = PDF::loadView('pages.pdf.bank_book_pdf', array('bank_book' => $bank_book, 'accountHead' => $accountHead, 'data' => $data));
            // $pdf->setPaper('A4', 'landscape');
            return $pdf->stream(Carbon::now() . '-recentstat.pdf');
        }
    }
