            ...
            <div class="card card-flush">
                ...
                <div class="card-header align-items-center py-5 gap-2 gap-md-5">
                    ...
                    <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                        ...
                        <!--end::Export dropdown-->
                        @can('create product')
                            <a href="{{ route('product_order.create') }}" class="btn btn-sm btn-primary">Add Product
                                Order</a>
                        @endcan
                    </div>
                    <!--end::Card toolbar-->
                    ...
                    <table class="table table-striped table-bordered align-middle table-row-dashed fs-7 gy-5 mb-0"
                        ...
                        <thead>
                            ...
                            <tr class="text-start fs-7 text-uppercase gs-0">
                                ...
                                <th class="min-w-70px">Order Category</th>
                                <th class="min-w-70px">Order Type</th>
                                <th class="min-w-90px">Manufacturer Article No</th>
                                <th class="min-w-90px">Customer Article No</th>
                                <th class="min-w-70px">Last No</th>
                        ...
                        <tbody class="fw-semibold text-gray-700">
                            @foreach ($productOrders as $productOrder)
                                <tr>
                                    ...
                                    <td>
                                        <a class="text-dark text-hover-primary">{{ $productOrder->customer_name }}</a>
                                    </td>
                                    <td>
                                        <div
                                            class="badge badge-light-{{ $productOrder->status == 1 ? 'success' : 'info' }}">
                                            {{ $productOrder->status == 1 ? 'Active' : 'Disabled' }}</div>
                                    </td>
                                    <td>{{ $productOrder->order_number }}</td>
                                    <td>{{ $productOrder->order_category }}</td>
                                    <td>{{ $productOrder->order_type }}</td>
                                    <td>{{ $productOrder->manufacturer_article_no }}</td>
                                    <td>{{ $productOrder->customer_article_no }}</td>
                                    <td>{{ $productOrder->last_no }}</td>
                                    ...
                                    <td class="text-end">
                                        ...
                                        <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4"
                                            ...
                                            <div class="menu-item px-3">
                                                @can('write product')
                                                    <a href="{{ route('product_order.edit', $productOrder->id) }}"
                                                        class="menu-link px-3">Edit</a>
                                                @endcan
                                                @can('read product')
                                                    <a href="{{ route('product_order.show', $productOrder->id) }}"
