                    ...
                    <div class="row mb-5">
                        <div class="col-md-2">
                            <div class="fv-row mb-0 fv-plugins-icon-container">
                                ...
                                <select class="form-select form-select-sm" data-control="select2" id="leave_year" name="leave_year" required>
                                    <option value="">Select Year</option>
                                    <?php
                                    $year_start = date('2023'); // 1990
                        ...
                        <div class="col-md-2">
                            <div class="fv-row mb-0 fv-plugins-icon-container">
                                <label class="fs-6 fw-semibold form-label mt-0">
                                    <span class="required">Select Section </span>
                                </label>
                                <select class="form-select form-select-sm" data-control="select2" id="changeSection" name="section_id" required>
                        ...
                        <div class="col-md-2">
                            <div class="fv-row mb-0 fv-plugins-icon-container">
                                <label class="fs-6 fw-semibold form-label mt-0">
                                    <span class="">Alternative Emp</span>
                                </label>
                                <select class="form-select form-select-sm" data-control="select2" id="selectAlternativeEmployee" name="alternative_employee_id">
                                    <option value="">Alternative Employee</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-md-2">
                            <div class="fv-row mb-0 fv-plugins-icon-container">
                                ...
                                <select class="form-select form-select-sm" data-control="select2" id="leave_type" name="leave_type"  required>
                        ...
                        @can('create leave entry')
                            ...
                            <div class="col-md-1 d-none" id="total_leave_div">
                                <div class="fv-row mb-7">
                                    <label class="fs-6 fw-semibold form-label mt-0">
                                        <span class="">Total Leave</span>
                                    </label>
                                    <!-- <p id="total_leave_days"></p> -->
                                    <input type="text" class="form-control form-control-sm form-control-solid" readonly
                                        name="total_leave_days" id="total_leave_days">
                                </div>
                            </div>
                            <div class="col-md-1 d-none" id="type_due_leave_div">
                                <div class="fv-row mb-7">
                                    <label class="fs-6 fw-semibold form-label mt-0">
                            ...
                            <div class="col-md-1 d-none" id="total_due_leave_div">
                                <div class="fv-row mb-7">
                                    <label class="fs-6 fw-semibold form-label mt-0">
...
<script type="text/javascript">
    ...
    // global declear var 
    var employeesListData;
    var employeeDetailsData;
    // on change section show employee list and leave entry list
    $('#changeSection').on('change', function() {
        ...
        $('#type_due_leave_div').addClass('d-none');
        $('#total_due_leave_div').addClass('d-none');

        var sectionID = $(this).val();
        if (sectionID) {
            $.ajax({
                url: 'leaveEntrySectionWiseEmployeeDetails/' + sectionID,
                type: "GET",
                data: {
                    "_token": "{{ csrf_token() }}"
                },
                ...
                success: function(response) {
                    employeesListData = response.employees;
                    employeeDetailsData = response.employeeDetails;
                    $('#selectEmployee').append(
                        `<option value="">Select Employee</option>`
                    );
                    // select employees list
                    if (employeesListData) {
                        $.each(employeesListData, function(key, value) {
                            $('#selectEmployee').append(
                    ...
                    // Leave entry table employees detailes list
                    if (employeeDetailsData) {
                        $('#leave_entry_div').addClass('d-none');
                        $('#leave_settings').empty();
                        $.each(employeeDetailsData, function(key, value) {
                            ...
                            $('#leave_settings').append(
                                `<tr>
                                    <td>
                                        <div class="d-flex">
                                            <div >
                                                <a id ="leave_id" class="text-gray-700 fs-6  mb-1" >${value.id}</a>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="d-flex">
                                            <div >
                                                <a class="text-gray-700 fs-6  mb-1"
                                                    >${value.employee_id ? value.employee_id.employee_name : ''}</a>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        ...
                                        <div class="d-flex">
                                            <div >
                                                <a id = "leave_id" class="text-gray-700 fs-6  mb-1"
                                                    >${value.alternative_employee_id?.employee_name || ''}</a>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
    ...
    // on change employee show totat deu and leave entry list
        ...
        if (employeeID) {
            $.ajax({
                ...
                success: function(leaveEntryDetails) {
                    if (leaveEntryDetails && leaveEntryDetails.length > 0) {
                        $('#leave_entry_div').removeClass('d-none'); // Show the leave entry div

                        // Clear existing content inside leave_settings before appending new data
                        $('#leave_settings').empty();
                        ...
                        $.each(leaveEntryDetails, function(key, value) {
                            ...
                            // leave entry form pass employee settings id
                            var leaveSettingId = value.leaveSettingsId;
                            // console.log(leaveSettingId);
                            $('#leaveSettingId').val(leaveSettingId);
                            ...
                            $('#leave_settings').append(
                                `<tr>
                                    <td>
                                        <div class="d-flex">
                                            <div>
                                                <a id="leave_id" class="text-gray-700 mb-1">${value.id}</a>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="d-flex">
                                            <div>
                                                <a class="text-gray-700 mb-1">${value.employee ? value.employee.employee_name : ''}</a>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                        ...
                        $('#leave_entry_div').addClass('d-none'); // Hide the leave entry div
                        $('#jsdataerror').text('Not Found'); // Display error message
                    }
                },
                error: function(xhr, status, error) {
                    // Handle AJAX errors
                    console.error(xhr.responseText);
                    $('#jsdataerror').text('Error: ' + error);
                }
            });
                        ...
                        $.each(data, function(key, value) {
                            if (initialLeave == 0) {
                                ...
                                if (leave_type == "Sick") {
                                    ...
                                    //console.log('sick_leave:', leaveTotal);
                                }
