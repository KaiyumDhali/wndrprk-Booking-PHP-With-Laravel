				...
				<div class="card card-flush">
					...
					<div class="card-header align-items-center py-5 gap-2 gap-md-5">
						...
						<div class="card-toolbar flex-row-fluid justify-content-end gap-5">
							...
							<!--end::Filter-->
							@can('create employee department')
                            <a href="{{ route('emp_department.create') }}" class="btn btn-primary btn-sm">Add Department</a>
							@endcan
						</div>
						<!--end::Card toolbar-->
					</div>
						...
						<table class="table table-striped table-bordered align-middle table-row-dashed fs-7 gy-5 mb-0"  id="kt_ecommerce_report_customer_orders_table" >
							...
							<thead>
								...
								<tr class="text-start fs-7 text-uppercase gs-0">
                                    ...
                                    <th class="min-w-100px">Department Status</th>
									@can('write employee department')
							...
							<tbody class="fw-semibold text-gray-700">
								...
								<tr>
									<td>
										<a>{{$department->branch_name}}</a>
									</td>
                                    <td>
										<a class="text-dark text-hover-primary">{{$department->department_name}}</a>
									</td>
                                    <td>
										<div class="badge badge-light-{{$department->status==1 ? 'success' : 'info'}}">{{ $department->status==1?'Active':'Disabled' }}</div>
									</td>
									@can('write employee department')
                                    <td class="text-end">
										<a href="{{ route('emp_department.edit', $department->id) }}"
											class="btn btn-sm btn-success hover-scale p-2" data-bs-toggle="tooltip" data-bs-placement="top" title="Edit"><i class="bi bi-check2-square fs-2 ms-1"></i></a>
                                    </td>
									@endcan
								</tr>
                                @endforeach
