...
<script type="text/javascript">
    ...
    function InvoiceList(startDate, endDate) {
        let url = "{{ route('requisition_invoice_date_search', ['startDate' => ':startDate', 'endDate' => ':endDate']) }}";
                ...
                if (response.invoices) {
                    ...
                    $.each(response.invoices, function(key, value) {
                        ...
                        if (statusValueApproved === approved_index) {
                            buttons += `
                                <a href="#" class="btn btn-sm btn-success hover-scale p-2 approve-btn" data-status-approved-url="${statusApprovedUrl}" data-bs-toggle="tooltip" data-bs-placement="top" title="Approved">
                        ...
                        table.row.add([rowIndex, invoiceNo, requisitionDate, statusText, buttons]).draw();
                    });
    ...
    $('#requisition_list').on('click', '.approve-btn', function(e) {
        e.preventDefault();
        let url = $(this).data('status-approved-url');
