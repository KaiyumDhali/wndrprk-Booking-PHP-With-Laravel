                                ...
                                <div class="row">
                                    ...
                                    <div class="col-12 col-md-12">
                                        ...
                                        <h3>New Booking</h3>
                                            ...
                                            <div class="row g-5 g-xl-8 px-10 py-10" style="max-height: 550px; overflow-x: scroll;">
                                                <div class="container">
...
<script type="text/javascript">
    ...
    if (rowExists) {
        ...
        let existingCheckIn = existingRow.find("td:eq(2)").text();
        let existingCheckOut = existingRow.find(".check-out-date").val();
        ...
        // Update the number of days and check-out+1 day
        updateDayCount(existingRow);
        updateNextDay(existingRow);
    } else {
        // If the room is not in the table, add a new row
        let nextDay = formatDate(addDays(checkOut, 1));
        ...
        <tr data-id="${id}">
            ...
            <td>${formatDate(checkIn)}<input type="hidden" name="table_check_in_date[]" value="${checkIn}"></td>
            <td><input type="date" class="form-control form-control-sm check-out-date" name="table_check_out_date[]" value="${checkIn}" data-room-id="${id}" data-check-in="${checkIn}"></td>
            <td class="next-day">${nextDay}</td>
...
// Function to add and display "Check-Out + 1 Day" in the row
function updateNextDay(row) {
    let checkOut = row.find(".check-out-date").val();
    let nextDay = formatDate(addDays(checkOut, 1));
    row.find(".next-day").text(nextDay);
}

// Event listener to update "Check-Out + 1 Day" when the check-out date changes
$(document).on('change', '.check-out-date', function() {
    ...
    return Math.max(1, Math.round(diffInDays)); // Ensures a minimum of 1 day
}
