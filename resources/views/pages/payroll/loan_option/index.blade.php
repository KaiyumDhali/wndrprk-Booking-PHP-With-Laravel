                    ...
                    <table class="table table-striped table-bordered align-middle table-row-dashed fs-7 gy-5 mb-0"
                        ...
                        <thead>
                            ...
                            <tr class="text-start fs-7 text-uppercase gs-0">
                                <th class="min-w-50px"> Loan Option Name</th>
                                <th class="min-w-100px"> Created By</th>
                                <th class="min-w-100px"> Loan Option Status</th>
                                @can('write loan option')
                        ...
                        <tbody class="fw-semibold text-gray-700">
                            ...
                            <tr>
                                ...
                                <td>
                                    <a class="text-dark text-hover-primary">{{$loanOption->created_by}} </a>
                                </td>
                                <td>
                                    <div class="badge badge-light-{{$loanOption->status==1 ? 'success' : 'info'}}">
                                        {{ $loanOption->status==1?'Active':'Disabled' }}</div>
                                </td>
                                @can('write loan option')
                                <td class="text-end">
                                    <button type="button" class="btn btn-sm btn-success p-2"
                                        data-loanoption="{{ json_encode($loanOption) }}"
                                        onclick="openEditModal(this)">Edit
                                        <i class="fa fa-pencil text-white px-2"></i>
                                    </button>
                                    <!-- <form method="POST" action="{{route('loan_option.destroy', $loanOption->id)}}">
                                        {{csrf_field()}}
                                        {{ method_field('DELETE') }}
            ...
            <div class="modal-content">
                ...
                <div class="modal-body scroll-y mx-5 mx-xl-15 my-7">
                    ...
                    <form id="kt_modal_new_card_form" class="form fv-plugins-bootstrap5 fv-plugins-framework"
                        method="POST" action="{{ route('loan_option.store') }}" enctype="multipart/form-data">
                        @csrf
                        <!--begin::Input group-->
                        <div class="d-flex flex-column mb-7 fv-row fv-plugins-icon-container">
                            <!--begin::Label-->
                            <label class="d-flex align-items-center fs-6 fw-semibold form-label mb-2">
                                <span class="required">Name Loan Option</span>
        ...
        <div class="modal-dialog modal-dialog-centered mw-650px">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body scroll-y mx-5 mx-xl-15 my-7">
                    <!--begin::Form-->
                    <form id="kt_modal_new_card_form" class="form fv-plugins-bootstrap5 fv-plugins-framework"
                        method="POST" action="{{ route('loan_option.update', ':loanOptionId') }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <!--begin::Input group-->
                        <div class="d-flex flex-column mb-7 fv-row fv-plugins-icon-container">
...
function openEditModal(button) {
    ...
    const loanOptionData = JSON.parse(button.getAttribute('data-loanoption'));
    ...
    const loanOptionId = loanOptionData.id;
    const updatedFormAction = formAction.replace(':loanOptionId', loanOptionId);
    ...
    $('#kt_modal_edit_card').find('input[name="name"]').val(loanOptionData.name);
    $('#kt_modal_edit_card').find('select[name="status"]').val(loanOptionData.status);
