                    ...
                    <table class="table table-striped table-bordered align-middle table-row-dashed fs-7 gy-5 mb-0"
                        ...
                        <thead>
                            ...
                            <tr class="text-start fs-7 text-uppercase gs-0">
                                <th class="min-w-50px"> Payslip Type Name</th>
                                <th class="min-w-100px"> Created By</th>
                                <th class="min-w-100px"> Payslip Type Status</th>
                                @can('write payslip type')
                        ...
                        <tbody class="fw-semibold text-gray-700">
                            ...
                            <tr>
                                <td>
                                    <a>{{$paysliptype->name}}</a>
                                </td>
                                <td>
                                    <a class="text-dark text-hover-primary">{{$paysliptype->created_by}}</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-{{$paysliptype->status==1 ? 'success' : 'info'}}">
                                        {{ $paysliptype->status==1?'Active':'Disabled' }}</div>
                                </td>
                                @can('write payslip type')
                                <td class="text-end">
                                    <button type="button" class="btn btn-sm btn-success p-2"
                                        data-paysliptype="{{ json_encode($paysliptype) }}"
                                        onclick="openEditModal(this)">Edit
                                        <i class="fa fa-pencil text-white px-2"></i>
                                    </button>
                                    <!-- <form method="POST" action="{{route('payslip_type.destroy', $paysliptype->id)}}">
                                        {{csrf_field()}}
                                        {{ method_field('DELETE') }}
            ...
            <div class="modal-content">
                ...
                <div class="modal-body scroll-y mx-5 mx-xl-15 my-7">
                    <form id="kt_modal_new_card_form" class="form fv-plugins-bootstrap5 fv-plugins-framework"
                        method="POST" action="{{ route('payslip_type.store') }}" enctype="multipart/form-data">
                        @csrf
                        <div class="d-flex flex-column mb-7 fv-row fv-plugins-icon-container">
                            <label class="d-flex align-items-center fs-6 fw-semibold form-label mb-2">
                                <span class="required">Name Payslip Type</span>
                        ...
                        <div class="text-center pt-15">
                            <a href="{{ route('payslip_type.index') }}" id="kt_modal_new_card_cancel"
    ...
    <div class="modal fade" id="kt_modal_edit_card" tabindex="-1" aria-hidden="true">
        ...
        <div class="modal-dialog modal-dialog-centered mw-650px">
            <div class="modal-content">
                ...
                <div class="modal-body scroll-y mx-5 mx-xl-15 my-7">
                    <form id="kt_modal_new_card_form" class="form fv-plugins-bootstrap5 fv-plugins-framework"
                        method="POST" action="{{ route('payslip_type.update', ':paysliptypeId') }}"
...
function openEditModal(button) {
    ...
    const paysliptypeData = JSON.parse(button.getAttribute('data-paysliptype'));
    ...
    const paysliptypeId = paysliptypeData.id;
    const updatedFormAction = formAction.replace(':paysliptypeId', paysliptypeId);
    ...
    $('#kt_modal_edit_card').find('input[name="name"]').val(paysliptypeData.name);
    $('#kt_modal_edit_card').find('select[name="status"]').val(paysliptypeData.status);
