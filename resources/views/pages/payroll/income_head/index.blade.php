            ...
            <div class="card card-flush">
                ...
                <div class="card-header align-items-center py-5 gap-2 gap-md-5">
                    ...
                    <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                        ...
                        <!-- <a href="{{ route('income_head.create') }}" class="btn btn-sm btn-primary btn-sm">Add
                            Income Head</a> -->

                        @can('create income head')
                    ...
                    <table class="table table-striped table-bordered align-middle table-row-dashed fs-7 gy-5 mb-0"
                        ...
                        <thead>
                            ...
                            <tr class="text-start fs-7 text-uppercase gs-0">
                                <th class="min-w-50px"> Income Head Name</th>
                                <th class="min-w-100px"> Created By</th>
                                <th class="min-w-100px"> Income Head Status</th>
                                @can('write income head')
                        ...
                        <tbody class="fw-semibold text-gray-700">
                            @foreach($allIncomeHeads as $allIncomeHead)
                            <tr>
                                <td>
                                    <a>{{$allIncomeHead->name}}</a>
                                </td>
                                <td>
                                    <a class="text-dark text-hover-primary">{{$allIncomeHead->created_by}}</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-{{$allIncomeHead->status==1 ? 'success' : 'info'}}">
                                        {{ $allIncomeHead->status==1?'Active':'Disabled' }}</div>
                                </td>
                                @can('write income head')
                                <td class="text-end">
                                    <button type="button" class="btn btn-sm btn-success p-2"
                                        data-allowanceoption="{{ json_encode($allIncomeHead) }}"
                                        onclick="openEditModal(this)">Edit
                                        <i class="fa fa-pencil text-white px-2"></i>
                                    </button>

                                    <!-- <form method="POST" action="{{route('income_head.destroy', $allIncomeHead->id)}}">
                                        {{csrf_field()}}
                                        {{ method_field('DELETE') }}
            ...
            <div class="modal-content">
                ...
                <div class="modal-body scroll-y mx-5 mx-xl-15 my-7">
                    ...
                    <form id="kt_modal_new_card_form" class="form fv-plugins-bootstrap5 fv-plugins-framework"
                        method="POST" action="{{ route('income_head.store') }}" enctype="multipart/form-data">
                        @csrf
                        <!--begin::Input group-->
                        <div class="d-flex flex-column mb-7 fv-row fv-plugins-icon-container">
                            <!--begin::Label-->
                            <label class="d-flex align-items-center fs-6 fw-semibold form-label mb-2">
                                <span class="required">Name Income Head</span>
                        ...
                        <div class="text-center pt-15">
                            ...
                            <a href="{{ route('income_head.index') }}" id="kt_modal_new_card_cancel"
        ...
        <div class="modal-dialog modal-dialog-centered mw-650px">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body scroll-y mx-5 mx-xl-15 my-7">
                    <!--begin::Form-->
                    <form id="kt_modal_new_card_form" class="form fv-plugins-bootstrap5 fv-plugins-framework"
                        method="POST" action="{{ route('income_head.update', ':allIncomeHeadId') }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <!--begin::Input group-->
                        <div class="d-flex flex-column mb-7 fv-row fv-plugins-icon-container">
...
function openEditModal(button) {
    ...
    const allIncomeHeadData = JSON.parse(button.getAttribute('data-allowanceoption'));
    ...
    const allIncomeHeadId = allIncomeHeadData.id;
    const updatedFormAction = formAction.replace(':allIncomeHeadId', allIncomeHeadId);
    ...
    $('#kt_modal_edit_card').find('input[name="name"]').val(allIncomeHeadData.name);
    $('#kt_modal_edit_card').find('select[name="status"]').val(allIncomeHeadData.status);
