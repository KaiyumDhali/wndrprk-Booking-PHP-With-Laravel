<x-default-layout>
    ...
    <div class="card card-flush h-lg-100 " id="employee_setting">
        ...
        <div class="card-body py-0">
                                ...
                                <div class="product fv-row fv-plugins-icon-container">
                                    <label class="form-check form-switch form-check-custom form-check-solid">
                                        <input class="form-check-input w-30px h-20px" type="checkbox"
                                            value="{{ $key }}" name="payroll_head_id[]" />
            ...
            <!-- Updated Modal with Payroll Heads Form -->
            <div class="modal fade" id="myModal" tabindex="-1" aria-labelledby="exampleModalLabel"
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <form id="payrollForm">
                            <div class="modal-body" id="payrollHeadsContainer">

                            </div>
                            <div class="modal-footer">
                                <button type="submit" class="btn btn-primary">Update Payroll Heads</button>
...
<script>
    ...
    function submitSelectedRows() {
        ...
        // Initialize selectedData as an empty array
        let selectedData = [];
        ...
        selectedRows.each(function() {
            let employeeId = $(this).closest('tr').find('[name="selected_users[]"]').val();

            // Get the selected payrollHeadId values for this row
            let selectedPayrollHeadIds = [];
            $('[name="payroll_head_id[]"]:checked').each(function() {
                selectedPayrollHeadIds.push($(this).val());
            });

            // Check if the combination of employeeId and payrollHeadId exists in selectedData
            let existingRow = selectedData.find(row => row.employee_id === employeeId);

            if (existingRow) {
                // If the combination exists, merge the new payrollHeadIds with existing ones
                existingRow.payroll_head_ids = existingRow.payroll_head_ids.concat(selectedPayrollHeadIds);
            } else {
                // If not, create a new row with the combination
                selectedData.push({
                    'employee_id': employeeId,
                    'payroll_head_ids': selectedPayrollHeadIds,
    ...
    $(document).ready(function() {
                ...
                success: function(data) {
                    ...
                    if (data.length > 0) {
                        ...
                        $.each(data, function(key, value) {
                            let d = data[key];
                            let update =
                                '<button type="button" class="btn btn-primary btn-sm update-btn" data-bs-toggle="modal" data-bs-target="#myModal" data-id="' +
                                d.id + '">Update</button>';
                            table.row.add([
                                ...
                                .designation_name, d.salary_section_name, update
                            ]);
                        });
                        table.draw();
                    } else {
        ...
        // Update modal content and attach event handler when a row is clicked
        $('#myModal').on('show.bs.modal', function(event) {
            var button = $(event.relatedTarget); // Button that triggered the modal
            var employee_id = button.data('id'); // Extract employee_id from data-id attribute

            let url = '{{ route('payroll_employee_edit', ['employee_id' => ':employee_id']) }}';
            url = url.replace(':employee_id', employee_id);

            // Populate payroll heads checkboxes
            $.ajax({
                url: url,
                type: "GET",
                data: {
                ...
                success: function(data) {
                    ...
                    $('#payrollHeadsContainer').empty();

                    // Append checkboxes for payrollHeadAdd
                    $.each(data.payrollHeadAdd, function(key, value) {
                        appendCheckbox(key, value);
                    });

                    // Function to append checkbox
                    function appendCheckbox(key, value) {
                        $('#payrollHeadsContainer').append(`
                        <div class="col pt-8">
                            <div class="product fv-row fv-plugins-icon-container">
                                <label class="form-check form-switch form-check-custom form-check-solid">
                                    <input class="payrollID form-check-input w-30px h-20px" type="checkbox" value="${key}" name="payroll_head_id[]" checked/>
                                    <span class="form-check-label text-muted fs-6">${value}</span>
                                </label>
                            </div>
                        </div>
        ...
        // Update modal content head un checkbox clicked
        $('#payrollForm').submit(function(event) {
            event.preventDefault();

            // Get unchecked checkboxes as an array of values
            const uncheckedCheckboxes = Array.from(document.querySelectorAll(
                    '#payrollForm input[type="checkbox"]:not(:checked)'))
            ...
            const url = '{{ route('payroll_employee_update') }}';
            ...
            $.ajax({
                ...
                success: function(response) {
                    $('#messageContainer').removeClass('alert-danger').addClass(
                            'alert-success').html('Success message here.')
                        .show();
                    setTimeout(function() {
                        $('#messageContainer').hide();
                    }, 5000);
                ...
                error: function(error) {
                    $('#messageContainer').removeClass('alert-success')
                        .addClass('alert-danger').html('Error message here.')
                        .show();
                    setTimeout(function() {
                        $('#messageContainer').hide();
                    }, 5000);
