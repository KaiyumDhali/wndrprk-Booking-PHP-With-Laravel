                                ...
                                <div class="row">
                                    ...
                                    <div id="bankDetails" class="bankDetails row p-0 m-0 d-none">
                                        <div class="col-12 col-md-3">
                                            <div class="card-body pt-0 pb-2">
                                                <div class="fv-row fv-plugins-icon-container">
                                                    <label class="">{{ __('Select Type') }}</label>
...
<script>
    ...
    document.getElementById("paymentVoucherform").addEventListener("keydown", function(event) {
    ...
    function InvoiceList(startDate, endDate, pdfdata) {
        let url;
        url =
            "{{ route('payment_voucher_date_search', ['startDate' => ':startDate', 'endDate' => ':endDate', 'pdf' => ':pdf']) }}";
                ...
                success: function(data) {
                    ...
                    if (data) {
                        ...
                        $.each(data, function(key, value) {
                            ...
                            let paymentVoucherUrl =
                                `{{ route('voucher_report_pdf', ['voucherNo' => '_voucherNo_', 'voucherType' => 'paymentVoucher']) }}`
                                .replace('_voucherNo_', voucherNo);
                            let debitVoucherUrl =
                                `{{ route('voucher_report_pdf', ['voucherNo' => '_voucherNo_', 'voucherType' => 'debitVoucher']) }}`
                                .replace('_voucherNo_', voucherNo);
                            let paymentJournalVoucherUrl =
                                `{{ route('voucher_report_pdf', ['voucherNo' => '_voucherNo_', 'voucherType' => 'paymentJournalVoucher']) }}`
                                .replace('_voucherNo_', voucherNo);
                        ...
                        `;
                            ...
                            let viewCell =
                                '<div class="d-flex flex-row" style="text-align: center;">' + view +
                                '</div>';

                            table.row.add([rowIndex, voucherNo, formatDate(voucherDate),
                                to_acc_name, to_acc_name_2, amountCell, viewCell
                            ]).draw();
                        });
