                        ...
                        <div class="d-flex align-items-center position-relative my-1 ">
                            <div class="card-body p-2">
                                <div class="product fv-row fv-plugins-icon-container">
                                    <h3>Total Supplier Payable List Report</h3>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <table class="table table-striped table-bordered align-middle table-row-dashed fs-7 gy-5 mb-0"
                        ...
                        <thead>
                            ...
                            <tr class="text-start fs-7 text-uppercase gs-0">
                                ...
                                <th class="min-w-100px text-end">Amount (BDT)</th>
                            </tr>
                            <!--end::Table row-->
                        </thead>
                        <!--end::Table head-->
...
<script type="text/javascript">
    ...
    var table = $('#supplier_payable_list').DataTable(dataTableOptions);

    function CallBack(pdfdata) {
        let url;
        url = '{{ route('total_supplier_payable_list_search', ['pdf' => ':pdf']) }}';
                ...
                success: function(data) {
                    ...
                    if (data.length > 0) {
                        ...
                        $.each(data, function(key, value) {
                            let d = data[key];
                            let amount = parseFloat(d.due) || 0;
                            table.row.add([id, d.account_name, d.account_mobile, `<div class="text-end">( ${formatCurrency(amount)} )</div>`
                            ]).draw();
                            id++;
                        });
