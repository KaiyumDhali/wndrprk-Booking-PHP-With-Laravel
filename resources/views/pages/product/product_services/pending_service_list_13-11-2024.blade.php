                    ...
                    <table class="table table-striped table-bordered align-middle table-row-dashed fs-7 gy-5 mb-0"
                        ...
                        <thead>
                            ...
                            <tr class="text-start fs-7 text-uppercase gs-0">
                                ...
                                <th style="width: 140px;">Service Number</th>
...
<script type="text/javascript">
    ...
    function InvoiceList(pdfdata) {
        ...
        url =
            '{{ route('pending_product_services_search', ['startDate' => ':startDate', 'endDate' => ':endDate', 'pdf' => ':pdf']) }}';
                ...
                success: function(data) {
                    ...
                    if (data) {
                        $.each(data, function(key, value) {
                            ...
                            // URLs
                            let serviceEntryUrl =
                                `{{ route('product_services_entry', ['id' => '_id_']) }}`
                                .replace('_id_', ID);
                            // Determine buttons
                            let buttons = `
                            <td class="text-right d-flex">
                                <a href="${serviceEntryUrl}" class="btn btn-sm btn-success hover-scale p-2 mx-2" data-bs-toggle="tooltip" data-bs-placement="top" title="Service Assign">
                                    Service Entry
                                </a>
                            </td>
                            `;
                            ...
                            let service_status = `
                                <td class="text-right d-flex">
                                    <p class="text-info">
                                        ${d.service_status == 0 ? 'Pending' : 'Assigned'}
                                    </p>
                                </td>
                            `;
                            // Add row to DataTable
                            let rowIndex = table.rows().count() + 1;
                            table.row.add([rowIndex, d.invoice_no, d.customer_name, d.product_name,
                                d.service_date, d.service_number, service_status, d.done_by,
                                buttons
                            ]).draw();
                        });
