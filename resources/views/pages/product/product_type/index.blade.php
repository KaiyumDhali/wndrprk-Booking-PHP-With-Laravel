                        ...
                        <tbody class="fw-semibold text-gray-700">
                            @foreach ($productTypes as $productType)
                                <tr>
                                    ...
                                    <td>
                                        <div
                                            class="badge badge-light-{{ $productType->status == 1 ? 'success' : 'info' }}">
                                            {{ $productType->status == 1 ? 'Active' : 'Disabled' }}</div>
                                    </td>
                                    @can('write product type')
                                        <td class="text-end">
                                            <form method="POST"
                                                action="{{ route('product_type.destroy', $productType->id) }}"
                                                class="delete-product-type-form">
                                                {{ csrf_field() }}
                                                {{ method_field('DELETE') }}
                                                <button type="button" class="btn btn-sm btn-success p-2"
                                                    data-productType="{{ json_encode($productType) }}"
            ...
            <div class="modal-content">
                ...
                <div class="modal-body scroll-y mx-5 mx-xl-15 my-7">
                    ...
                    <form id="kt_modal_new_card_form" class="form fv-plugins-bootstrap5 fv-plugins-framework"
                        method="POST" action="{{ route('product_type.store') }}" enctype="multipart/form-data">
                        @csrf
                        <!--begin::Input group-->
                        ...
                        <div class="text-center pt-15">
                            <a href="{{ route('product_type.index') }}" id="kt_modal_new_card_cancel"
    ...
    <div class="modal fade" id="kt_modal_edit_card" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered mw-650px">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body scroll-y mx-5 mx-xl-15 my-7">
                    <!--begin::Form-->
                    <form id="kt_modal_edit_card_form_product_type"
                        class="form fv-plugins-bootstrap5 fv-plugins-framework" method="POST"
                        action="{{ route('product_type.update', ':productTypeId') }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <!--begin::Input group-->
                        <div class="d-flex flex-column mb-7 fv-row fv-plugins-icon-container">
...
<script type="text/javascript">
    ...
    function openEditModalProductType(button) {
        ...
        const formAction = $('#kt_modal_edit_card').find('#kt_modal_edit_card_form_product_type').attr('action');
        const productTypeId = productTypeData.id;
        const updatedFormAction = formAction.replace(':productTypeId', productTypeId);
        $('#kt_modal_edit_card').find('#kt_modal_edit_card_form_product_type').attr('action', updatedFormAction);

        $('#kt_modal_edit_card').find('input[name="type_name"]').val(productTypeData.type_name);
