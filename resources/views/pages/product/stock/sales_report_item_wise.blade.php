            ...
            <div class="card card-flush">
                ...
                <div class="card-body">
                    <div class="row d-flex">
                        ...
                        <div class="col-md-2 py-2">
                            <div class="fv-row mb-0 fv-plugins-icon-container">
                                <label>{{ __('Select Customer Name') }}</label>
                                <select id="account_name" class="form-control form-control-sm" name="account_name" data-control="select2" data-hide-search="false" required>
                                    <option selected selected value="0">Select Customer Name</option>
...
<script type="text/javascript">
    ...
    function CallBack(pdfdata) {
        ...
        url =
            '{{ route('item_wise_sales_search', ['startDate' => ':startDate', 'endDate' => ':endDate', 'productID' => ':productID', 'accountName' => ':accountName', 'pdf' => ':pdf']) }}';
                ...
                success: function(data) {
                    ...
                    if (data.length > 0) {
                        ...
                        $.each(data, function(key, value) {
                            let d = data[key];
                            // 'invoice_no', 'voucher_no', 'voucher_date', 'transaction_date', 'account_name', 'account_mobile', 'account_address', 'narration', 'type', 'balance_type', 'amount'
                            let type = '';
                            if (d.type === 'SV') {
                            ...
                            table.row.add([formatDate(d.transaction_date), particulars, type, d.voucher_no, debitCell, creditCell
                            ]).draw();
                        });
