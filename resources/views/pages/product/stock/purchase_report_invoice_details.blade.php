                            ...
                            <div class="d-flex flex-column gap-5 gap-lg-5">
                                <div class="d-flex flex-column flex-xl-row gap-5 gap-lg-5">
                                    ...
                                    <div class="card card-flush flex-row-fluid overflow-hidden pt-3"
                                        ...
                                        <!--end::Card header-->
                                        <!--begin::Card body-->
                                        <div class="card-body py-0 pt-2">Supplier ID :
                                            {{ $stocks[0]->supplier_finance_account->id }}
                                    ...
                                    <div class="card card-flush py-0 flex-row-fluid overflow-hidden"
                                        ...
                                        <div class="card-body pt-5">
                                            ...
                                            <a href="{{ route('purchase_invoice_details_Pdf', $stocks[0]->invoice_no) }}"
                                                class="btn btn-sm btn-primary px-5 my-3"
                                                target="_blank">{{ __('PDF Download') }}</a>
                                            <h4 class="mb-0"> Invoice No (#{{ $stocks[0]->invoice_no }})
                                        ...
                                        <div class="table-responsive">
                                            ...
                                            <table
                                                ...
                                                <tbody class="fw-semibold text-gray-600">
                                                    <!--begin::Products-->
                                                    <span
                                                        class="d-none">{{ $initialTotal = 0, $initialDiscount = 0 }}</span>
                                                    @foreach ($stocks as $stock)
                                                        <tr class="text-center">
                                                            <td class="">{{ $loop->iteration }}</td>
                                                            <td class="">{{ $stock->product_id }}</td>
                                                            <td class="text-start">{{ $stock->product->product_name }}
                                                            </td>
                                                            <td class="text-center">
                                                                {{ $stock->product->unit->unit_name }}</td>
                                                            <td class="">{{ $stock->stock_in_quantity }}</td>
                                                            <td class="text-end">
                                                                {{ formatCurrency($stock->stock_in_unit_price) }}</td>
                                                            <td class="text-end">
                                                                {{ formatCurrency($stock->stock_in_discount) }}</td>
                                                            <td class="text-end">
                                                                {{ formatCurrency($productTotal = $stock->stock_in_total_amount) }}
                                                            </td>
                                                            <span
