            ...
            <div class="card card-flush">
                <div class="card-header align-items-center py-5 gap-2 gap-md-5">
                    ...
                    <div class="card-title">
                        <h3>Invoice Wise Purchase Report</h3>
                    </div>
                    <!--begin::Card toolbar-->
...
<script type="text/javascript">
    ...
    function InvoiceList(pdfdata) {
        ...
        url = '{{ route('purchase_invoice_date_search', ['startDate' => ':startDate', 'endDate' => ':endDate', 'pdf' => ':pdf']) }}';
                ...
                success: function(data) {
                    ...
                    if (data) {
                        $.each(data, function(key, value) {
                            ...
                            // URLs
                            let returnUrl =
                                `{{ route('purchase_invoice_return', ['invoiceNo' => '_invoiceNo_']) }}`
                                .replace('_invoiceNo_', invoiceNo);
                            let editUrl =
                                `{{ route('purchase_invoice_edit', ['invoiceNo' => '_invoiceNo_']) }}`
                                .replace('_invoiceNo_', invoiceNo);
                            let showUrl =
                                `{{ route('purchase_invoice_details', ['invoiceNo' => '_invoiceNo_']) }}`
                                .replace('_invoiceNo_', invoiceNo);
                            let pdfUrl =
                                `{{ route('purchase_invoice_details_Pdf', ['data' => '_invoiceNo_']) }}`
                                .replace('_invoiceNo_', invoiceNo);
