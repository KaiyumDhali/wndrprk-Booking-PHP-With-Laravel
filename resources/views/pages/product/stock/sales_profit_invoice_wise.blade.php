            ...
            <div class="card card-flush">
                <div class="card-header align-items-center py-2 gap-2 gap-md-5">
                    ...
                    <div class="card-title">
                        <h3>Invoice Wise Sales Profit</h3>
                    </div>
                    <!--begin::Card toolbar-->
                    ...
                    <table class="table table-striped table-bordered align-middle table-row-dashed fs-7 gy-5 mb-0"
                        ...
                        <thead>
                            ...
                            <tr class="fs-7 text-uppercase gs-0">
                                ...
                                <th class="text-center w-40px">Invoice No</th>
                                <th class="text-center w-80px">Quantity</th>
                                <th class="text-center w-100px">Purchase Price</th>
                                <th class="text-center w-100px">Discount</th>
                                <th class="text-center w-100px">Sales Price</th>
                                <th class="text-center w-100px">Profit</th>
                                <th class="text-center w-40px">Action</th>
                            </tr>
                            <!--end::Table row-->
                        </thead>
                        <!--end::Table head-->
...
<script type="text/javascript">
    ...
    function CallBack(pdfdata) {
        ...
        url =
            '{{ route('invoice_wise_profit_search', ['startDate' => ':startDate', 'endDate' => ':endDate', 'pdf' => ':pdf']) }}';
                ...
                success: function(data) {
                    ...
                    if (data.length > 0) {
                        ...
                        $.each(data, function(key, value) {
                            ...
                            let purchase_price = '<div style="text-align: right;">' + formatCurrency(d.net_purchase_price) + '</div>';
                            // let purchase_price = '<div style="text-align: right;">' + d.net_purchase_price + '</div>';
                            let net_discount = '<div style="text-align: right;">' + formatCurrency(d.net_discount) + '</div>';
                            let sales_price = '<div style="text-align: right;">' + formatCurrency(d.net_sales_price) + '</div>';
                            let profit = '<div style="text-align: right;">' + formatCurrency(d.net_profit) + '</div>';
                           
                            // URLs
                            let iteamProfitUrl =
                                `{{ route('invoice_wise_profit_sales_details', ['invoiceNo' => '_invoiceNo_']) }}`
                                .replace('_invoiceNo_', invoiceNo);
                            
                            let buttons = `
                                <td class="text-center">
                                    <a href="${iteamProfitUrl}" class="text-center btn btn-sm btn-primary hover-scale p-2 mx-2" target="_blank">
                                        Iteam Profit
                                    </a>
                                </td>
                            `;

                            table.row.add([d.sales_date, d.invoice_no, d.quantity, purchase_price, net_discount, sales_price, profit, buttons]).draw();
                        });
