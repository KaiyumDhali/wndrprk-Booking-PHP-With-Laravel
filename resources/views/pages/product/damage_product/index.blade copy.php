                    ...
                    <table class="table table-bordered align-middle table-row-dashed fs-6 gy-5"
                        ...
                        <thead class="bg-gray-200">
                            ...
                            <tr class="text-start fw-bold fs-7 text-uppercase gs-0">
                                ...
                                {{-- <th style="width: 120px">Is Exchangeable</th>
                                <th style="width: 120px">Is Repairable</th>
                                <th style="width: 120px">Is Resaleable</th> --}}
                        ...
                        <tbody class="fw-semibold text-gray-600">
                            @foreach ($damage_lists as $damage_list)
                                <tr>
                                    <td>{{ $damage_list->id }}</td>
                                    <td>{{ $damage_list->damage_no }}</td>
                                    <td>{{ $damage_list->product->product_name }}</td>
                                    <td>{{ $damage_list->warehouse->name }}</td>
                                    <td>{{ $damage_list->supplier->account_name }}</td>
                                    <td>{{ $damage_list->purchasePrice }}</td>
                                    <td>{{ $damage_list->salePrice }}</td>
                                    <td>{{ $damage_list->damage_quantity }}</td>
                                    <td>{{ $damage_list->damage_reason }}</td>
                                    <td>{{ $damage_list->damage_date }}</td>
                                    <td>{{ $damage_list->status }}</td>
                                    <td>{{ $damage_list->done_by }}</td>


                                    {{-- <td>{{ $damage_list->product->type_name }}</td>
                                    <td>{{ $damage_list->product->category_name }}</td>
                                    <td>{{ $damage_list->purchase_price }}</td>
                                    <td>{{ $damage_list->sale_price }}</td>
                                    <td>{{ $damage_list->damage_quantity }}</td>
                                    <td>{{ $damage_list->supplier_id }}</td>
                                    <td>{{ $damage_list->damage_reason }}</td>
                                    <td>{{ $damage_list->damage_date }}</td>
                                    <td>{{ $damage_list->is_exchangeable ? 'Yes' : 'No' }}</td>
                                    <td>{{ $damage_list->is_repairable ? 'Yes' : 'No' }}</td>
                                    <td>{{ $damage_list->is_resaleable ? 'Yes' : 'No' }}</td> --}}

                                    <td class="text-end">
                                        <div class="d-flex flex-row-reverse">
                                            {{-- @can('read damage') --}}
                                            <a class="btn btn-sm btn-info py-2 mx-1"
                                                href="{{ route('damage.show', $damage_list->id) }}">View</a>
                                            {{-- @endcan
                                            @can('write damage') --}}
                                            <a class="btn btn-sm btn-success py-2 mx-1"
                                                href="{{ route('damage.edit', $damage_list->id) }}">Edit</a>
                                            {{-- @endcan --}}
                                        </div>
                                    </td>
                                </tr>
...
<script type="text/javascript">
    ...
    //     @if (session('message'))
    //         Swal.fire({
    //             title: '{{ session('alert-type') === 'success' ? 'Saved!' : 'Error!' }}',
