                ...
                <div class="card-header align-items-center py-5 gap-2 gap-md-5">
                    ...
                    <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                        ...
                        @can('create sub category')
                            <a href="{{ route('sub_category.create') }}" class="btn btn-primary btn-sm">Add Sub Category</a>
                        @endcan
                    </div>
                    <!--end::Card toolbar-->
                </div>
                        ...
                        <tbody class="fw-semibold text-gray-700">
                            @foreach ($sub_categories as $sub_category)
                                <tr>
                                    ...
                                    <td>
                                        <a class="text-dark text-hover-primary">{{ $sub_category->category_name }}</a>
                                    </td>
                                    <td>
                                        <a
                                            class="text-dark text-hover-primary">{{ $sub_category->sub_category_name }}</a>
                                    </td>
                                    <td>
                                        <div
                                            class="badge badge-light-{{ $sub_category->status == 1 ? 'success' : 'info' }}">
                                            {{ $sub_category->status == 1 ? 'Active' : 'Disabled' }}</div>
                                    </td>
                                    @can('write sub category')
                                        <td class="text-end">
