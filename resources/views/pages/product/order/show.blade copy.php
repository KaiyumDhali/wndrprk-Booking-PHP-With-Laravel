                        ...
                        <div class="d-flex flex-column gap-5 gap-lg-5">
                            <div class="d-flex flex-column flex-xl-row gap-5 gap-lg-5">
                                ...
                                <div class="card card-flush flex-row-fluid overflow-hidden"
                                    ...
                                    <!--end::Card header-->
                                    <!--begin::Card body-->
                                    <div class="card-body py-0">Customer ID : {{$orders->customer->id}}
                                        <br>Customer Name : {{$orders->customer->customer_name}}
                                        <br>Address : {{$orders->customer->customer_address}}
                                        <br>Mobile No : {{$orders->customer->customer_mobile}}
                                    </div>
                                    <!--end::Card body-->
                                </div>
                                <!--end::Payment address-->
                                <!--begin::Shipping address-->
                                <div class="card card-flush py-5 flex-row-fluid overflow-hidden"
                                     style="text-align: right">
                                    <div class="card-body pt-5">
                                        <h2 class="mb-0"> Order No (#{{$orders->order_no}}) </h2>
                                        <br>Order Date : {{$orders->order_date}}
...
//                                                        dd($orders);
                                            ...
                                            <tr class="text-center">
                                                ...
                                                <td class="text-end"> {{number_format($order->stock_out_unit_price, 2)}}</td>
                                                <td class="text-end"> {{number_format($order->stock_out_discount, 2)}}</td>
                                                <td class="text-end"> {{ number_format($productTotal =($order->stock_out_quantity * $order->stock_out_unit_price),2) }} </td>
                                            <span class="d-none">{{ $initialTotal = $initialTotal + $productTotal, $initialDiscount = $initialDiscount + $order->stock_out_discount }}</span>
                                            </tr>
                                                @endforeach
                                            <!--end::Products-->
                                            <!--begin::Subtotal-->
                                            <tr class="">
                                                <td colspan="6" class="fs-5 text-dark text-end ">Subtotal</td>
                                            ...
                                            <!--begin::Grand total-->
                                            <tr>
                                                <td colspan="6" class="fs-4 text-dark text-end">Net Amount</td>
                                                <td colspan="6" class="text-dark fs-4 fw-bolder text-end">
                                                        {{ number_format($initialTotal - $initialDiscount, 2)}}
                                                </td>
                                            </tr>
                                            {{-- <tr>
                                                <td colspan="6" class="fs-4 text-dark text-end">Pay Amount</td>
                                                <td colspan="6" class="text-dark fs-4 fw-bolder text-end">
                                                        {{ number_format($customerPayment,2) }} 
                                                </td>
                                            </tr>
                                            <tr>
                                                <td colspan="6" class="fs-4 text-dark text-end">Due Amount</td>
                                                <td colspan="6" class="text-dark fs-4 fw-bolder text-end">
                                                        {{ number_format(($initialTotal - $initialDiscount) - $customerPayment, 2) }}
                                                </td>
                                            </tr> --}}
                                            <tr>
                                                ...
                                                <td colspan="" class="fs-4 text-dark text-end">
                                                    @if ($orders->file !== null)
                                                    <img src="{{ Storage::url($orders->file) }}" height="200"
                                                         width="" alt="Attach File" />
                                                    @endif
                                                </td>
                                                <td colspan="2" class="fs-4 text-dark text-end">
                                                </td>
