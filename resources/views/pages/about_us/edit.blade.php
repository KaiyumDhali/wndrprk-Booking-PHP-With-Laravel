                    ...
                    <form action="{{ route('about_us.update', $about_us->id) }}" method="POST"
                        ...
                        <div class="mb-5">
                            ...
                            <input type="text" class="form-control form-control-solid" id="title" name="title"
                                value="{{ old('title', $about_us->title) }}" />
                            {{-- <textarea class="form-control form-control-solid" id="title" name="title" rows="2">{{ old('title', $about_us->title) }}</textarea> --}}
                        </div>
                        ...
                        <div class="mb-5">
                            ...
                            <textarea class="form-control" id="description" name="description" rows="5">{{ old('description', $about_us->description) }}</textarea>
                        </div>
                        ...
                        <div class="mb-5">
                            ...
                            @if ($about_us->image)
                                <div class="mt-2">
                                    <img src="{{ asset(Storage::url($about_us->image)) }}" height="75" width="75"
                                        alt="About Us Image" />

                                    @if ($about_us->image != null)
                                        <a class="btn btn-sm btn-danger"
                                            onclick="return confirm('Are you sure you want to delete ?')"
                                            href="{{ route('about_us.image_destroy', $about_us->id) }}"><i
                        ...
                        <div class="mb-5">
                            ...
                            <select class="form-select form-select-solid" id="status" name="status">
                                ...
                                <option value="1" {{ $about_us->status == 1 ? 'selected' : '' }}>Active</option>
                                <option value="0" {{ $about_us->status == 0 ? 'selected' : '' }}>Disable</option>
                            </select>
                        </div>

                        <div class="d-flex justify-content-end my-5">
                            <!--begin::Button-->
                            <a href="{{ route('about_us.index') }}" id="kt_ecommerce_add_product_cancel"
